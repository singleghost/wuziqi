...
class Ws{
	...
	public $clients = array(); //考虑添加一个客户端的类
	/*
	...
	*/
	...
	public function start_server() {
		...
		while(TRUE) {
			...
			//阻塞用，有新连接时才会结束
			if( false === socket_select($this->cycle, $write, $except, null)) {
				echo "socket_select() error. reason: " . socket_strerror(socket_last_error()) . "\n";
			}
			foreach ($this->cycle as $k => $v) {
				...
				if (!@socket_recv($v, $data, 1024, 0) || !$data) {//没消息的socket就跳过
					echo "close socket $v with subscript $k";
					$this->close($v);
					continue;
				}
	...
	private function add_accept($accept) {
        ...
        $this->clients[$index] = new Client($accept, $index); 
	}
